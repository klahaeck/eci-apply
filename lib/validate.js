...
export const validateSubmission = (program, submission) => {
  ...
  if (!contacts) {
    ...
    contacts.forEach(contact => {
      if (!contact.name || !contact.email || !contact.phone_number || !contact.address0 || !contact.city || !contact.state || !contact.zip || !contact.identifyAs) {
  ...
  if (!details || details.length !== questions.length) {
    errors.details = 'You must answer all questions';
  }
  ...
  if (assets.length < minWorkAssets) {
    errors.assets = `You must add at least ${minWorkAssets} assets`;
  }

  if (assets.length > maxWorkAssets) {
    errors.assets = `You must remove ${assets.length - maxWorkAssets} assets`;
  }
