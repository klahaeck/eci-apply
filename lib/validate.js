...
export const validateSubmission = (program, submission) => {
  ...
  const errors = {};
  const { minWorkAssets, maxWorkAssets } = program;
  const { contacts, resume, statement, proposal, assets } = submission;
  ...
  if (!contacts) {
    ...
    contacts.forEach(contact => {
      if (!contact.name || !contact.email || !contact.phone_number || !contact.address0 || !contact.city || !contact.state || !contact.zip) {
  ...
  if (!statement) {
    errors.statement = 'A statement is required';
  }

  if (!proposal) {
    errors.proposal = 'A proposal is required';
  }
