...
const getProjection = (role) => {
  ...
  return role === 'admin' ? {...defaultFields,
    ...
    'eligible': 1,
    'ratingScopes':  { '$first': '$ratingScopes.scopes' },
    'myRating': { '$first': '$ratingScopes.rate' },
    'myNotes': 1
  } : defaultFields;
};


export const getSubmissions = async ({ programId, _id, userId, role, searchQuery, sortBy, sortOrder, perPage, pageNumber }) => {
